...
Trend Discovery — Monitors YouTube trends and finds viral content in your niche
Channel Monitoring — Tracks selected channels for new high-performing videos
Auto Download — Downloads videos automatically using yt-dlp
Transcription — Transcribes audio to text for content analysis
AI Analysis — Uses Mistral AI to identify the most viral moments
Clip Cutting — Cuts and crops clips to vertical 9:16 format (YouTube Shorts ready)
Metadata Generation — Auto-generates titles, descriptions, and hashtags
Telegram Approval — Sends each clip to your Telegram bot for review before upload
YouTube Upload — Uploads approved clips automatically with smart scheduling
📋 Features
Core Pipeline
...
✅ Clip preview with video + buttons in a single message
...
✏️ EDIT TITLE — lets you change the title before approving
🗄 Database Schema
Table	Description
...
🔒 Security
All API keys are stored in config/settings.json (git-ignored)
YouTube OAuth tokens are stored in config/credentials/ (git-ignored)
The SQLite database is git-ignored
A settings.example.json template is provided for safe sharing
📊 Clip Pipeline — Status Flow
text

pending_clip
    ↓ (Clipper — every 15 min)
ready_to_upload
    ↓ (Metadata — every 30 min)
pending_approval
    ↓ (Telegram — you press APPROVE)
approved
    ↓ (Uploader — every 10 min)
uploaded
🧠 AI Stack
...
⚠️ Disclaimer
This tool is intended for creating original short-form content from legally obtained sources. Always ensure you have the right to use, transform, and republish any content you process. Respect YouTube Terms of Service and copyright laws in your jurisdiction.
